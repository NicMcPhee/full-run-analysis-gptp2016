...
This is the simplified version of individual 20:484; it should probably be put in a figure:
\begin{verbatim}
(\space
 \newline
 in1
...
This is actually a pretty ``human'' program, and is similar to how a person
might implement replace-space-with-newline using Push. The first five lines
(maybe put these together on a line?) replace all the spaces with newlines
(using the \texttt{string\_replacechar} instruction) and prints the result,
thereby solving half the problem. The next four lines remove all the spaces from
a fresh copy of the input string, compute the lenth and leave that on the
